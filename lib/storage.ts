  ...
  return {
    ...
    scenes: reorderScenes(
      (project.scenes ?? []).map((scene) => ({
        ...
        srtStartOffset: Number.isFinite(scene.srtStartOffset) ? scene.srtStartOffset : -0.2,
        srtEndHold: Number.isFinite(scene.srtEndHold) ? Math.max(0, scene.srtEndHold ?? 0) : 0.35,
