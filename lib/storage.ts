  ...
  return {
    ...
    scenes: reorderScenes(
      (project.scenes ?? []).map((scene) => ({
        ...
        note: scene.note ?? "",
        visualNotes: scene.visualNotes ?? scene.note ?? "",
        sfxNotes: scene.sfxNotes ?? "",
        status: scene.status ?? "empty",
        srtCueStartId: scene.srtCueStartId ?? null,
        srtCueEndId: scene.srtCueEndId ?? null,
        srtCueStartIndex: scene.srtCueStartIndex ?? null,
        srtCueEndIndex: scene.srtCueEndIndex ?? null,
        srtStartOffset: Number.isFinite(scene.srtStartOffset) ? scene.srtStartOffset : 0,
        srtEndHold: Number.isFinite(scene.srtEndHold) ? Math.max(0, scene.srtEndHold ?? 0) : 0,
